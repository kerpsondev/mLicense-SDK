...
public class CreateOperation implements Operation<OperationResult<Boolean>> {

  private final String url;
  private final MSecrets secrets;
  private final ModuleObject moduleObject;

  public CreateOperation(String url, MSecrets secrets, ModuleObject moduleObject) {
    this.url = url;
  ...
  private HttpBuilder prepareRequest() {
    return HttpBuilder.put()
        .url(this.url)
        .body(this.moduleObject.buildJsonForCreate())
