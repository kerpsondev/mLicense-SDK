...
public class LicenseUpdateOperation implements Operation<OperationResult<Boolean>> {

  private final String url;
  private final MSecrets secrets;
  private final License license;

  public LicenseUpdateOperation(String url, MSecrets secrets, License license) {
    this.url = url;
  ...
  private HttpBuilder prepareRequest() {
    return HttpBuilder.post()
        .url(this.url)
        .body(this.license.buildJsonForUpdate())
  ...
  public OperationResult<Boolean> complete() {
    try {
      ...
      if (response.statusCode() == 409) {
        return new OperationResult<>(false, new LicenseUpdateException("License with this key already exists."));
      }

      if (response.statusCode() == 423) {
        return new OperationResult<>(false, new LicenseUpdateException("You don't have any plan assigned or you have reached the limit of licenses."));
      }
